...
bool FileDialog::IsFileTypeAllowed(const std::filesystem::path& path) const {
	if (m_allowed_extensions.empty()) return true;
...
void FileDialog::RenderPathBar() {
	...
	for (auto it = paths.rbegin(); it != paths.rend(); ++it) {
		...
		if (ImGui::Button(it->filename().string().empty() ? it->root_name().string().c_str()
	...
	for (const auto* entry_ptr : sorted_entries) {
		...
		try {
			if (entry.is_directory()) {
				if (ImGui::Selectable((ICON_LC_FOLDER "  " + filename).c_str(), false,
				...
				if (icon.empty()) icon = ICON_LC_FILE;
				...
				if (ImGui::IsItemHovered()) {
					...
					ImGui::Text("Size: %.2f KB",
						static_cast<float>(std::filesystem::file_size(path)) / 1024.0f);
					ImGui::EndTooltip();
				}
			}
		}
...
FileDialog::FileDialog(bool& show_dialog, std::string& selected_file, const std::vector<std::string>& allowed_extensions)
	: m_show_dialog(show_dialog)
	, m_selected_file(selected_file)
